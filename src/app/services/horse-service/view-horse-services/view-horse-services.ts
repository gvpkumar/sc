...
import { DialogHorseService } from '../dialogService';

@Component({
  selector: 'view-horse-services',
  styleUrls: ['view-horse-services.scss'],
  templateUrl: 'view-horse-services.html',
})
export class HorseServicesComponent {

  isLoggedIn: boolean = false;
  getData;
  pageEvent: PageEvent;

  displayedColumns = ['customColumn1', 'horseNumber','horseServiceDate','horseServiceNextDueDate','horseServiceServicedby','horseServiceRequestedBy', 'edit'];
  ...
  openDialog(id): void {
     console.log(id);
    let dialogRef = this.dialog.open(DialogHorseService, {
        width: '400px',
        height: '160px', 
        data:{id :id}
    });
...
export class ExampleHttpDao {
  ...
  getCustomUrlsCreatedList(): Observable<customUrlInfo[]> {
    ...
    return this.userService.getHorseServiceData();
