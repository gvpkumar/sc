...
import { DialogTrailerService } from '../dialogService';
@Component({
  selector: 'view-trailer-services',
  styleUrls: ['view-trailer-services.scss'],
  templateUrl: 'view-trailer-services.html',
})
export class TrailerServicesComponent {

  isLoggedIn: boolean = false;
  getData;
  pageEvent: PageEvent;

  displayedColumns = ['customColumn1', 'trailerNumber','trailerServiceDate','trailerServiceNextDueDate','trailerServiceServicedby','trailerServiceRequestedBy', 'edit'];
  ...
  openDialog(id): void {
    console.log(id);
   let dialogRef = this.dialog.open(DialogTrailerService, {
       width: '400px',
       height: '160px', 
       data:{id :id}
   });
...
export class ExampleHttpDao {
  ...
  getCustomUrlsCreatedList(): Observable<customUrlInfo[]> {
    ...
    return this.userService.getTrailerServiceData();
