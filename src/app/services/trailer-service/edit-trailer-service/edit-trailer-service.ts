...
@Component({
  ...
  selector: 'edit-trailer-service',
  styleUrls: ['edit-trailer-service.scss'],
  templateUrl: 'edit-trailer-service.html',
})
export class EditTrailerServiceComponent implements OnInit {
  ...
  editDetails(id: String): any {
    ...
    this.userService.getTrailerServiceDetails(id)
      .subscribe(
        data => {
          this.getData = data;
  ...
  update(): any {
    ...
    this.userService.updateTrailerServiceDetails(this.getData)
      .subscribe(
        data => {
          this.success = true;
