...
import { DialogTrailerComponent } from '../dialogService';
import { MatDialog, MatDialogRef, MAT_DIALOG_DATA } from '@angular/material';

@Component({
  selector: 'trailers',
  styleUrls: ['view-trailers.scss'],
  templateUrl: 'view-trailers.html',
})
export class TrailersComponent {
  ...
  displayedColumns = ['customColumn1', 'trailerNumber', 'fitness', 'roadTax', 'identity',
  ...
  openDialog(id): void {
    console.log(id);
    let dialogRef = this.dialog.open(DialogTrailerComponent, {
      width: '400px',
      height: '160px',
      data: { id: id }
    });
...
export class ExampleHttpDao {
  ...
  getCustomUrlsCreatedList(): Observable<customUrlInfo[]> {
    ...
    return this.userService.getTrailerData();
